  ...
  <header
    ...
    :class="[
      isScrolled
        ...
        : 'bg-white/90 backdrop-blur-sm shadow-2xs border-b border-slate-200/60'
    ]"
  >
    ...
    <div v-if="settingsStore.isTopAnnouncementActive" class="bg-navy-950 text-slate-200 text-[11px] sm:text-xs py-1 px-3 sm:px-8 border-b border-navy-900 announcement-bar">
      <div class="max-w-7xl mx-auto flex items-center justify-between gap-2">
        ...
        <div class="flex items-center gap-2 sm:gap-4 ms-auto text-xs shrink-0">
          ...
          <button
            type="button"
            class="flex items-center gap-1 text-gold-400 hover:text-gold-300 font-bold px-1.5 sm:px-2 py-0.5 rounded bg-navy-900 border border-navy-800 hover-lift btn-press transition-colors text-[10px] sm:text-xs"
    ...
    <div class="max-w-7xl mx-auto px-3 sm:px-8 py-2.5 sm:py-3 flex items-center justify-between gap-2 sm:gap-4">
      ...
      <div class="flex items-center gap-1.5 sm:gap-2.5 shrink-0">
        ...
        <button
          type="button"
          class="flex items-center gap-1.5 px-2 sm:px-3 py-1.5 sm:py-2 text-xs sm:text-sm text-slate-500 bg-slate-100 hover:bg-slate-200/80 rounded-xl transition-colors border border-slate-200 hover-lift btn-press"
        ...
        <router-link
          v-if="modulesStore.isModuleEnabled('admissions')"
          to="/admissions"
          class="hidden sm:inline-flex items-center gap-1.5 px-3.5 sm:px-4 py-1.5 sm:py-2 text-xs sm:text-sm font-bold text-navy-950 bg-gold-400 hover:bg-gold-300 rounded-xl shadow-sm hover:shadow-gold-glow hover-lift btn-press transition-all duration-200"
          @click="closeAllDropdowns"
        >
        ...
        <button
          type="button"
          class="lg:hidden p-1.5 sm:p-2 text-slate-700 hover:bg-slate-100 rounded-xl transition-colors shrink-0 btn-press"
    ...
    >
      <div
        v-if="mobileMenuOpen"
        class="lg:hidden border-t border-slate-200 bg-white px-4 pt-3 pb-6 space-y-2 max-h-[80vh] overflow-y-auto shadow-xl"
      >
        <!-- Always Present: Home Link -->
        <router-link
          to="/"
          class="block px-4 py-2.5 rounded-xl text-base font-semibold text-slate-700 hover:bg-slate-100 hover:text-navy-950 transition-colors"
        ...
        <div v-for="group in navGroups" :key="'mob-' + group.id" class="space-y-1">
          ...
          <router-link
            v-if="group.items.length === 1"
            :to="group.items[0].to"
            class="flex items-center gap-2 px-4 py-2.5 rounded-xl text-base font-semibold text-slate-700 hover:bg-slate-100 hover:text-navy-950 transition-colors"
            @click="mobileMenuOpen = false"
          >
            ...
            >
              <div v-if="openMobileGroups.has(group.id)" class="px-3 py-2 space-y-1 bg-white">
                <router-link
                  ...
                  :to="item.to"
                  class="flex items-center gap-2.5 px-3 py-2 rounded-lg text-sm font-medium text-slate-700 hover:bg-slate-100 hover:text-navy-950 transition-colors"
                  @click="mobileMenuOpen = false"
                >
        ...
        <router-link
          to="/admin"
          class="block px-4 py-2.5 rounded-xl text-base font-semibold text-gold-700 bg-gold-50 hover:bg-gold-100 transition-colors"
          @click="mobileMenuOpen = false"
        >
        ...
        <div v-if="modulesStore.isModuleEnabled('admissions')" class="pt-3 border-t border-slate-100 flex flex-col gap-2">
          <router-link
            to="/admissions"
            class="w-full text-center py-3 font-bold text-navy-950 bg-gold-400 hover:bg-gold-300 rounded-xl shadow-sm hover-lift btn-press transition-all"
            @click="mobileMenuOpen = false"
          >
            {{ $t('nav.applyNow') }}
          </router-link>
